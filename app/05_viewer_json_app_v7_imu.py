...
# -*- coding: utf-8 -*-
"""
05 Viewer — Sensor Dashboard (v7-IMU)
- 좌측: "데이터 추가" 버튼만, 파일명만 표시
- 하단: Meta / Labels 제목 크게·굵게(가운데 정렬), 표는 last key만 노출
- 상단(IMU 전용): 2×2 그리드, 각 패널은 영문 지표명(+단위) 제목, L/R 막대 + 값 라벨
- 하단 왼쪽 여백에 'IMU 지표 설명' 텍스트 블록(한국어) 고정 표기
- Pad / Insole 시각화는 이전 버전 그대로 유지
"""
...
APP_TITLE = "05 Viewer — Sensor Dashboard (v7-IMU)"
...
# ---------- plotting ----------
# IMU: show these in order (titles in EN with units)
IMU_ORDER = [
    "gait_cycle",
    "knee_flexion_max",
    "knee_extension_max",
...
class PlotPane(ttk.Frame):
    ...
    def _imu_dashboard(self, values: Dict[str, Any]):
        ...
        # 여백(아래쪽)에 지표 설명 텍스트 블록 추가
        # rect로 하단 12% 정도 공간 확보
        self.fig.tight_layout(rect=[0, 0.12, 1, 1])
        self.fig.text(
            0.01, 0.02,
            IMU_DESC_KO,
            fontsize=10,
