...
function drawGauge(el, raw, min, max) {
  if (!el) {
    console.warn('drawGauge: 요소를 찾을 수 없음', el);
    return;
  }
  
  console.log(`drawGauge: ${el.id}`, { raw, min, max });
  ...
  console.log(`drawGauge: ${el.id} 정규화된 값`, { L, R });
  ...
  const setTxt = (id, v) => { 
    ...
    if(s) {
      s.textContent=fmt1(v);
      console.log(`drawGauge: ${id} = ${fmt1(v)}`);
    } else {
      console.warn(`drawGauge: ${id} 요소를 찾을 수 없음`);
    }
  };
  ...
  const addDot = (cls, val) => {
    const n = toNum(val); 
    if (n===null) {
      console.log(`drawGauge: ${cls} 값이 null/undefined`);
      return;
    }
    const p = pct(n, min, max); 
    if (p===null) {
      console.log(`drawGauge: ${cls} 백분율 계산 실패 (${n}, ${min}, ${max})`);
      return;
    }
    ...
    bg.appendChild(d);
    console.log(`drawGauge: ${cls} 포인터 추가 (${n}, ${p}%)`);
  };
...
function initApp() {
  ...
  function updateIMUData() {
    ...
    console.log("IMU 메트릭 값들:", {
      gait_cycle: imuData.gait_cycle,
      knee_flexion_max: imuData.knee_flexion_max,
      knee_extension_max: imuData.knee_extension_max,
      foot_clearance: imuData.foot_clearance
    });
  ...
  function updatePadData() {
    ...
    console.log("Gait Pad 메트릭 값들:", {
      step_length: padData.step_length ?? padData.step_len,
      velocity: padData.velocity,
      stance_phase_rate: padData.stance_phase_rate,
      swing_phase_rate: padData.swing_phase_rate,
      double_support_time: padData.double_support_time
    });
  ...
  function clearAllUI() {
    ...
    console.log('UI 완전 초기화 완료');
  }

  // IMU 섹션 초기화
  function clearIMUSection() {
    const imuMetrics = ['gait_cycle', 'knee_flexion_max', 'knee_extension_max', 'foot_clearance'];
    imuMetrics.forEach(metric => {
      // 값 초기화
      const lEl = document.getElementById(`imu_${metric}_L`);
      const rEl = document.getElementById(`imu_${metric}_R`);
      if (lEl) lEl.textContent = '–';
      if (rEl) rEl.textContent = '–';
      
      // 게이지 바 초기화
      const gaugeEl = document.getElementById(`imu_${metric}`);
      if (gaugeEl) {
        gaugeEl.innerHTML = '';
        const bg = document.createElement("div");
        bg.className = "bar-bg";
        gaugeEl.appendChild(bg);
      }
    });
  }

  // Gait Pad 섹션 초기화
  function clearPadSection() {
    const padMetrics = ['step_length', 'velocity', 'stance_phase_rate', 'swing_phase_rate', 'double_support_time'];
    padMetrics.forEach(metric => {
      // 값 초기화
      const lEl = document.getElementById(`pad_${metric}_L`);
      const rEl = document.getElementById(`pad_${metric}_R`);
      if (lEl) lEl.textContent = '–';
      if (rEl) rEl.textContent = '–';
      
      // 게이지 바 초기화
      const gaugeEl = document.getElementById(`pad_${metric}`);
      if (gaugeEl) {
        gaugeEl.innerHTML = '';
        const bg = document.createElement("div");
        bg.className = "bar-bg";
    ...
    // 보행주기 비율 바 초기화
    const phaseLEl = document.getElementById('pad_phase_L');
    const phaseREl = document.getElementById('pad_phase_R');
    if (phaseLEl) phaseLEl.innerHTML = '';
    if (phaseREl) phaseREl.innerHTML = '';
  }

  // Smart Insole 섹션 초기화
  function clearInsoleSection() {
    // Day 칩 초기화
    ...
    const insoleMetrics = ['gait_speed', 'balance', 'foot_pressure_rear', 'foot_pressure_mid', 'foot_pressure_fore', 'gait_distance', 'stride_length', 'foot_angle'];
    insoleMetrics.forEach(metric => {
      // 값 초기화
      const lEl = document.getElementById(`ins_${metric}_L`);
      const rEl = document.getElementById(`ins_${metric}_R`);
      if (lEl) lEl.textContent = '–';
      if (rEl) rEl.textContent = '–';
      
      // 게이지 바 초기화
      const gaugeEl = document.getElementById(`ins_${metric}`);
      if (gaugeEl) {
        gaugeEl.innerHTML = '';
        const bg = document.createElement("div");
        bg.className = "bar-bg";
  ...
  // Meta/Labels 초기화
  function clearMetaLabels() {
    const metaTbody = document.getElementById('meta_tbody');
    const labelsTbody = document.getElementById('labels_tbody');
